...
def addSNMP(cgx, data):
    ...
    for element in elem_list:
        ...
        if name == None:
            name = "Unamed device"
        if not sid is None:
            snmp_resp = cgx.get.snmptraps(site_id=sid,element_id=elem_id)
            snmp_resp = snmp_resp.cgx_content.get('items', None)
            if snmp_resp:
                for snmp in snmp_resp:
                    print(snmp)
                    if snmp['description'] == data['description']:
                        print("Please delete SNMP Trap on element: " + name + " before trying to add or modify settings")
                    else:
                        resp = cgx.post.snmptraps(site_id=sid, element_id=elem_id, data=data)
                        if not resp:
                            print("Error creating SNMP Trap on " + name)
                        else:
                            print("Created SNMP Trap on " + name)
            else:
                 resp = cgx.post.snmptraps(site_id=sid, element_id=elem_id, data=data)
                 if not resp:
                     print("Error creating SNMP Trap on " + name)
            ...
            if snmp_resp:
                for snmp in snmp_resp:
                    ...
                    if snmp_description == description:
                        resp = cgx.delete.snmptraps(site_id=sid, element_id=elem_id, snmptrap_id=snmp_id)
                        if not resp:
                            print("Error deleting SNMP Trap on " + name)
                        else:
                            print("Deleted SNMP Trap with description: " + description + " on " + name )
    return True, "200"
...
def go():
    ...
    config_group.add_argument("--destroy", help="DESTROY SNMP traps",
                              default=False, action="store_true")
    config_group.add_argument("--destroyall", help="DESTROY all SNMP traps",
                              default=False, action="store_true")
    config_group.add_argument("--get", help="Get SNMP traps",
                              default=False, action="store_true")
    ...
    cgx = cgx_session
    data = {"description":"test","tags":None,"enabled":"true","server_ip":"1.1.1.1","source_interface":None,"version":"v2","v2_config":{"community":"public"},"v3_config":None}
